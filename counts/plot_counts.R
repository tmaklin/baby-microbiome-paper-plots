...
pdf(file = "counts_differences_no_saur.pdf", width = 8, height = 6)
layout(matrix(c(1, 3, 2, 3), nrow = 2, ncol = 2), heights = c(0.85, 0.15), widths = c(0.5, 0.5))
par(mar = c(5, 4, 0.5, 1))
## panel a
plot(y = n.clusters:1, x = diffs/colSums(obs.mat), xaxt = 'n', yaxt = 'n', ylab = '', xlab = "Relative difference", cex = 2, col = ifelse(diffs < 0, "#92c5de", "#f4a582"), pch = 18, bty = 'n', cex.lab = 1.2)
...
## panel b
plot(y = n.clusters:1, x = diffs, xaxt = 'n', yaxt = 'n', ylab = '', xlab = "Absolute difference", cex = 2, col = ifelse(diffs < 0, "#92c5de", "#f4a582"), pch = 18, bty = 'n', cex.lab = 1.2, xlim = c(-250, 250))
abline(v = 0, col = "gray80", lty = "dashed")
axis(side = 2, at = n.clusters:1, labels = clusters, las = 2, font = 3, cex.axis = 1.2)
x.axis.labels <- c(paste('-', seq(100, 25, by = -25), '%', sep=''), "0%", paste('+', seq(25, 100, by = 25), '%', sep=''))
axis(side = 1, at = seq(-250, 250, by = 50), cex.axis = 1.25)
...
legend("bottom", legend = c("Caesarean section cohort", "Vaginal delivery cohort"), pch = rep(18, 2), cex = 1.2, ncol = 1, bty = 'n', col = c("#92c5de", "#f4a582"))
title("Enrichened in", font.main = 1, cex.main = 1.5, adj = 0.5, line = -1.5)
dev.off()
