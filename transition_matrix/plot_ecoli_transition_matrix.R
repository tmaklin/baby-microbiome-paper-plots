...
pdf(file = "E_coli_transition_matrix_first_month.pdf", width = 8, height = 5)
layout(matrix(c(1, 2, 3), ncol = 3, byrow = TRUE), widths = c(2, 1.5, 0.35), heights = c(0.65, 0.9))
par(mar = c(0, 8, 8, 0.5))
PlotTransitionMatrix(vaginal.mats$no_mother_no_infancy, "a)", 0.07, -5, 17, n.max, 0.3, gradient)
par(mar = c(7.5, 6, 8, 1))
PlotTransitionMatrix(caesarean.mats$no_mother_no_infancy, "b)", 0.565, -5, 9.6, n.max, 0.25, gradient)
## par(mar = c(0, 8, 8, 0.5))
...
par(mar = c(1, 0, 8, 4))
n.max <- 10
IntensityLegend(gradient, n.max, -24)
dev.off()
