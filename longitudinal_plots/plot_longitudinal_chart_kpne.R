...
PersistencePlot <- function(input.data) {
    ...
    ##    clusters.to.plot <- rev(clusters.to.plot[order(as.numeric(sub("K_[a-z]*_ST([0-9]*).*$", "\\1", clusters.to.plot)))])
    clusters.to.plot <- rev(clusters.to.plot[order(clusters.to.plot)])
...
##    plotting.data <- plotting.data[plotting.data$Individual %in% names(has.a.switch)[unlist(has.a.switch)], ]
    ...
    axis(side = 2, at = 1:n.clusters.to.plot, labels = gsub("_", " ", gsub("Pop", "SC", gsub("_SC[0-9]*", "", gsub("K_", "K. ", clusters.to.plot)))), las = 2, cex.axis = 1.5)
...
caesarean <- ExtractCohortBySpecies(demix.check.data, full.metadata, "Caesarean", "K_[a-z]*")
vaginal <- ExtractCohortBySpecies(demix.check.data, full.metadata, "Vaginal", "K_[a-z]*")
...
pdf(file = "Klebsiella_persistence.pdf", width = 20, height = 8)
